...
pub fn ingest_inventory<P: AsRef<Path>>(path: P) -> AppResult<serde_json::Value> {
    ...
    let decrypted_bytes = match decrypt(&ciphertext) {
        ...
        Err(e) => {
            ...
            let mut start_idx = 0;
            while start_idx < ciphertext.len()
                && (ciphertext[start_idx].is_ascii_digit()
                    || ciphertext[start_idx].is_ascii_whitespace())
            {
                start_idx += 1;
            }
            if start_idx < ciphertext.len()
                && (ciphertext[start_idx] == b'{' || ciphertext[start_idx] == b'[')
            {
                ciphertext[start_idx..].to_vec()
            } else {
                return Err(format!(
                    "Decryption failed and file does not look like plain JSON: {e:?}"
                )
                .into());
            }
        }
    ...
    let inventory_value =
        if let Some(inventory_str) = outer_json.get("InventoryJson").and_then(|v| v.as_str()) {
