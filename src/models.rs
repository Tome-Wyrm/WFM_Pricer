...
pub struct WfmStatsItem {
    ...
    /// WFM's statistics API sends this as `mod_rank`, not `rank` — the alias below is
    /// required; without it every row silently deserializes as `rank: None`, breaking
    /// rank-filtered price/volume queries.
